...
import { 
  ...
  TrendingUp, History, UserCheck, UserX
} from 'lucide-react';
...
export default function AdminDashboardOverview({ setActiveTab }) {
  ...
  useEffect(() => {
    ...
    // SECURITY HANDSHAKE: Listening to all public activity
  ...
  async function fetchStats() {
    try {
      ...
      // CORE CHANGE: Using (!p.is_approved) to catch FALSE and NULL (Social signups)
      setPendingUsers(staff.filter(p => !p.is_approved));
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col w-full font-sans text-slate-900 overflow-x-hidden">
      ...
      {/* HEADER SECTION (EXACT ALIGNMENT MAINTAINED) */}
      <header className="flex flex-col md:flex-row md:items-center justify-between px-8 py-6 gap-4">
        ...
        <div className="flex items-center gap-3 flex-1 md:justify-end relative">
          ...
          <div className="relative flex items-center gap-2 pl-3 ml-1 border-l border-slate-200">
             <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="w-10 h-10 rounded-xl bg-indigo-600 text-white font-black flex items-center justify-center hover:shadow-lg transition-all active:scale-95 italic">RH</button>
             {showProfileMenu && (
                <div className="absolute right-0 mt-36 w-48 bg-white border border-slate-100 rounded-[24px] shadow-2xl z-[100] p-2 animate-in slide-in-from-top-2">
                   <button className="w-full flex items-center gap-3 p-3 hover:bg-slate-50 rounded-xl text-[10px] font-black uppercase tracking-widest"><Settings size={14} /> Matrix Settings</button>
                   <button onClick={handleLogout} className="w-full flex items-center gap-3 p-3 hover:bg-red-50 rounded-xl text-red-500 text-[10px] font-black uppercase tracking-widest mt-1 italic"><LogOut size={14} /> Exit Session</button>
                </div>
             )}
             <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest hidden sm:block italic ml-1 uppercase">ADMIN HQ</span>
          </div>
        </div>
      </header>

      <main className="flex-1 px-8 pb-10 space-y-6">
        {/* STAT TILES MODULE (NO DESIGN CHANGES) */}
        ...
        {/* QUICK ACTIONS MODULE (NO DESIGN CHANGES) */}
      ...
      {/* NOTIFICATION HUB MODAL (RABNAWAZ ACCESS COMMAND CENTER) */}
          ...
          <div className="bg-white w-full max-w-2xl rounded-[35px] shadow-2xl overflow-hidden flex flex-col border border-white animate-in zoom-in-95 duration-200">
             <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-[#F8FAFC]">
               <div>
                  ...
                  <p className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.3em] mt-1 italic">Authentication Registry</p>
               </div>
               ...
               {pendingUsers.map((user, i) => (
                 <div key={i} className="flex items-center justify-between p-5 bg-slate-50 rounded-[24px] border border-slate-100 group transition-all hover:bg-white hover:shadow-xl">
                   <div className="flex gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-white border border-slate-100 flex items-center justify-center font-black italic text-indigo-600 text-[10px] shadow-sm">ID</div>
                      <div>
                        <p className="text-[13px] font-black text-slate-800 uppercase italic tracking-tighter leading-none mb-1">{user.full_name || 'UNIDENTIFIED OPERATIVE'}</p>
                        <p className="text-[10px] font-bold text-slate-400 lowercase italic opacity-80">{user.email}</p>
                      </div>
                   </div>
                   <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300">
                     <button onClick={() => handleUserDecision(user.id, 'ALLOW')} className="w-10 h-10 bg-indigo-600 text-white rounded-xl flex items-center justify-center hover:bg-indigo-700 active:scale-90 transition-all shadow-lg shadow-indigo-100">
                        <Check size={18} />
                     </button>
      ...
      {/* RESOLUTION LEDGER MODAL (UNCHANGED) */}
      {showHistoryModal && (
...
// ---------------- STYLE REPLICATORS (STRICTLY IMMUTABLE) ---------------- //
